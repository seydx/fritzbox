...
exports.request = async (uri, options) => {
  try {
    ...
    if (err.response) {
      throw new Error(`Invalid response: ${uri} - ${err.response.statusMessage} (${err.response.statusCode})`);
    } else if (err.request) {
      throw new Error(`No or invalid response: ${uri} - ${err.message} (${err.code})`);
    } else {
      throw new Error(err);
    }
  }
